...
var spgc = $.extend({}, spgc);/* 定义全局对象，类似于命名空间或包的作用 */
 ...
 */
spgc.modalDialog = function (options) {
    if (spgc.modalDialog.handler == undefined) {// 避免重复弹出
        var opts = $.extend({
            title: '',
            width: 840,
            height: 680,
            modal: true,
            onClose: function () {
                spgc.modalDialog.handler = undefined;
                $(this).dialog('destroy');
            },
            onOpen: function () {
        ...
        return spgc.modalDialog.handler = $('<div/>').dialog(opts);
    }
};
 ...
 *            回调方法
 */
spgc.messagerConfirm = function (title, msg, fn) {
 ...
 * @requires jQuery,EasyUI
 */
spgc.messagerShow = function (options) {
 ...
 * @requires jQuery,EasyUI
 */
spgc.messagerAlert = function (title, msg, icon, fn) {
 ...
 *            主题名称
 */
spgc.changeTheme = function (themeName) {
 ...
 * @returns 对应名称的值
 */
spgc.getUrlParam = function (name) {
 ...
 * @returns object
 */
spgc.serializeObject = function (form) {
    var o = {};
